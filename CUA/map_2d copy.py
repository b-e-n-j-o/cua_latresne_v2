...
# 🆕 Chemin du catalogue : variable d'environnement ou chemin par défaut
CATALOGUE_PATH = os.getenv("CATALOGUE_PATH")
if not CATALOGUE_PATH:
    # Chemin par défaut : ../catalogues/catalogue_couches_map.json (depuis CUA/)
    default_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)),  # Remonter d'un niveau (CUA -> cua_latresne_v4)
        "catalogues",
...
if not os.path.exists(CATALOGUE_PATH):
    logger.error(f"❌ Fichier catalogue introuvable : {CATALOGUE_PATH}")
    logger.error("💡 Vérifiez que le fichier existe ou définissez la variable d'environnement CATALOGUE_PATH")
    raise FileNotFoundError(f"Fichier catalogue introuvable : {CATALOGUE_PATH}")
