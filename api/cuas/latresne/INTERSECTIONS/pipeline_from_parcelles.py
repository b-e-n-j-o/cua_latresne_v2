...
# Racine projet cua_latresne_v4 (…/api/cuas/latresne/INTERSECTIONS → 4 niveaux)
_PROJECT_ROOT = Path(__file__).resolve().parents[4]
if str(_PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(_PROJECT_ROOT))

from api.cuas.latresne.CERFA_ANALYSE.verification_unite_fonciere import verifier_unite_fonciere
from api.cuas.latresne.INTERSECTIONS.intersections import (
    CATALOGUE,
    calculate_intersection,
    fetch_superficie_indicative,
    format_intersection_layer,
)
from api.cuas.latresne.INTERSECTIONS.intersection_modules.enrichment import enrich_intersections_rapport
from api.cuas.latresne.CUA.sub_orchestrator_cua import generer_visualisations_et_cua_depuis_wkt
from api.cuas.latresne.CERFA_ANALYSE.auth_utils import is_authorized_for_insee
