...
2️⃣ Génère la visualisation 3D figée (MNT + contour UF) à partir du même WKT
...
from api.cuas.latresne.CUA.map2d.carte2d.carte2d_rendu import generer_carte_2d_depuis_wkt
from api.cuas.latresne.CUA.map3d.terrain_frozen import exporter_visualisation_3d_from_wkt
from api.cuas.latresne.CUA.docx.cua_builder import run_builder
from api.cuas.latresne.INTERSECTIONS.export_gpkg_intersections import export_gpkg_from_wkt
from api.cuas.argeles.carto_context.carto_context_urls import friendly_carto_url
...
def _storage_content_type(remote_path: str) -> str:
    lower = (remote_path or "").lower()
...
def upload_to_supabase(local_path, remote_path, bucket=None):
    ...
    supabase.storage.from_(bucket).upload(
        ...
        content,
        {
            "content-type": _storage_content_type(remote_path),
...
def generer_visualisations_et_cua_depuis_wkt(
    ...
    # ÉTAPE 2 : CARTE 3D
    # --------------------------------------------------------
    logger.info("\n📦 ÉTAPE 2/5 : Génération de la visualisation 3D figée (MNT)")
    path_3d = None
    path_dxf = None
    metadata_3d = None
    url_3d = None
    url_dxf = None
    if not skip_3d:
        res3d = exporter_visualisation_3d_from_wkt(
            wkt_path=wkt_path,
            output_dir=OUT_DIR,
            exaggeration=1.5
        )
        ...
        if path_dxf:
            logger.info(f"✅ DXF topo généré : {path_dxf}")
    else:
    ...
    url_2d_storage = upload_to_supabase(html_2d_path, remote_2d)
    url_3d_storage = None
    url_dxf = None
    if path_3d and not skip_3d:
        url_3d_storage = upload_to_supabase(path_3d, remote_3d)
    if path_dxf and Path(path_dxf).exists() and not skip_3d:
        remote_dxf = f"{remote_dir}/topo_mnt.dxf"
        url_dxf = upload_to_supabase(path_dxf, remote_dxf)

    commune_slug = (commune or "latresne").strip().lower() or "latresne"
    url_2d = friendly_carto_url(commune_slug, slug)
    url_3d = f"{url_2d}?vue=3d" if url_3d_storage else None
    gc.collect()
    log_memory("APRES_UPLOADS")
    log_memory_detailed("UPLOADS_DONE")

    logger.info(f"🌐 URL publique 2D : {url_2d} (storage {url_2d_storage})")
    ...
    payload = {"carte2d": url_2d, "carte3d": url_3d, "commune": commune_slug}
    token = base64.urlsafe_b64encode(json.dumps(payload).encode()).decode()
    maps_page_url = url_2d
    ...
    try:
        ...
        if url_dxf and path_dxf and Path(path_dxf).exists():
            register_project_file(
                supabase_client=supabase,
                slug=slug,
                file_kind="topo_dxf",
                filename="topo_mnt.dxf",
                storage_path=f"{remote_dir}/topo_mnt.dxf",
                public_url=url_dxf,
                storage_bucket=SUPABASE_BUCKET,
                mime_type="application/dxf",
                size_bytes=Path(path_dxf).stat().st_size,
                uploaded_by=user_id,
                source="pipeline_output",
            )
