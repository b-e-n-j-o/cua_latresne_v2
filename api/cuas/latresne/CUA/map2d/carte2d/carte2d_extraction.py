...
from api.cuas.latresne.CUA.map2d.map_utils import get_layers_on_buffer
...
SUPABASE_PORT = str(os.getenv("SUPABASE_PORT", "5432")).strip().strip('"').strip("'")
...
def charger_catalogue(catalogue_path: str | None = None):
    ...
    if not catalogue_path:
        ...
        # CUA/map2d/carte2d/ → api/cuas/latresne/catalogues/catalogue_couches_map.json
        racine_latresne = Path(__file__).resolve().parent.parent.parent.parent
