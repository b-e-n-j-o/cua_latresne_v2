...
# Catalogue identité foncière
PPRI_LAYER_TABLE_KEY = "pm1_detaillee_gironde"

# Couleurs par `nom_code` (PPRI Latresne)
...
def color_for_nom_code(nom_code: Any) -> str:
    n = _norm_nom_code(nom_code)
    return NOM_CODE_HEX.get(n, NOM_CODE_HEX[None])


def _uf_wkt_4326(parcelle_gdf: gpd.GeoDataFrame) -> str:
    g = unary_union(parcelle_gdf.geometry)
    return g.wkt
...
def compute_ppri_pct_by_codezone(
    ...
    """% de surface UF par `codezone` (somme = 100 si recouvrement complet)."""
        ...
        with conn.cursor() as cur:
            ...
            for cz, area in cur.fetchall():
                if area and float(area) > 0:
                    ...
                    stats[key] = stats.get(key, 0.0) + float(area)
    finally:
        conn.close()
    return {k: (v / total_area) * 100.0 for k, v in stats.items() if v > 0}


def fetch_ppri_map_gdf(
    parcelle_gdf: gpd.GeoDataFrame,
    buffer_m: float,
    geo_table: str = PPRI_GEO_TABLE,
) -> gpd.GeoDataFrame:
    """Entités PPRI découpées au buffer (carto), EPSG:3857."""
    ...
    if not rows:
        return gpd.GeoDataFrame(columns=["codezone", "nom_code", "geometry"], crs="EPSG:3857")
...
def _color_map_for_codezones(ppri_gdf: gpd.GeoDataFrame, pct_stats: Dict[str, float]) -> Dict[str, str]:
    out: Dict[str, str] = {}
    for cz in pct_stats:
        ck = str(cz).strip()
        sub = ppri_gdf[ppri_gdf["codezone"].astype(str).str.strip() == ck]
        if sub.empty:
            out[ck] = NOM_CODE_HEX[None]
            continue
        nc = sub.iloc[0].get("nom_code")
...
def fetch_ppri_laius_par_codezones(
    ...
    """Textes `laius_reglement` (Markdown) par `codezone` depuis `pm1_detaillee_gironde`."""
...
def build_ppri_section_flowables(
    ...
    *,
    ...
    """Contenu PDF : encart titre, lien règlement, image, laius par zone."""
        ...
        for z_key in sorted(non_vides.keys(), key=lambda x: str(x).lower()):
            ...
            tbl = Table(
                [
                    [
                        Paragraph(
                            f'<font color="#1e4d2f"><b>Zone {zt}</b> — PPRI</font>',
                            ps["ppri_zone_title"],
                        )
                    ],
                    [inner_tbl],
...
def generate_ppri_visuals_from_uf_geometry(
    ...
    *,
    ...
    geo_table: str = PPRI_GEO_TABLE,
) -> Optional[Tuple[str, Dict[str, float], List[dict[str, Any]]]]:
    """
    Retourne (chemin PNG, stats % par codezone, détail parcelles) si l’UF intersecte le PPRI ;
    sinon None (aucune page / image).
    """
    out = Path(out_dir)
    ...
    pct_stats = compute_ppri_pct_by_codezone(parcelle_gdf, geo_table=geo_table)
    if not pct_stats:
        return None

    ppri_gdf = fetch_ppri_map_gdf(parcelle_gdf, buffer_m, geo_table=geo_table)
    color_map = _color_map_for_codezones(ppri_gdf, pct_stats)
    ...
    return map_path, pct_stats, parcelles_detail
