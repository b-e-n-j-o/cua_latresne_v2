...
#    bash services/ingestion/ingestion_cadastre/scripts/render_cron_etl_latresne.sh
#
# B) Cron Job qui appelle l'API web (si le service web est toujours actif) :
#    bash services/ingestion/ingestion_cadastre/scripts/render_cron_etl_latresne.sh --http
#
...
exec python services/ingestion/ingestion_cadastre/run_etl_all_communes.py
